...
export class ScenarioController {
  ...
  async runScenario(req: Request, res: Response): Promise<void> {
    try {
      ...
      const allEntries = simulationHistory.getAll();
      const newEntry: HistoryEntry = {
        id: recordId,
        runNumber: allEntries.length + 1,
        name: `Run ${allEntries.length + 1} - ${scenarioConfig.id}`,
  ...
  async runSimulation(req: Request, res: Response): Promise<void> {
    try {
      ...
      const newEntry: HistoryEntry = {
        id: recordId,
        runNumber: allEntries.length + 1,
        name: `Run ${allEntries.length + 1} - ${validScenario}`,
